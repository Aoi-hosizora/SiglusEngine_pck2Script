...
int main(int argc,char* argv[])
{
	...
	FILE* txt = fopen(argv[2],"rb"); // .txt
	FILE* script = fopen(argv[1],"rb"); // .ss
	...
	for (ulong x=0;x<header->string_index_pair.count;x++) { // decrypt string
		...
		//////////////////////////////////////////////////////////////////////////
		decrypt_string(info_str,new_str,info->length,x); // xor

		string_table_t *item = new string_table_t;
		item->old_string = new_str;
		item->new_string = 0;
		string_table.push_back(item); // vec
	}
